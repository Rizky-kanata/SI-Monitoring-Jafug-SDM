...
{
    if (typeof src === 'string' && /(?:^|\/)js\/(extensions|stencils)\.min\.js(?:$|\?)/.test(src))
    {
        // Work around intermittent Cloudflare 522 on exact asset URLs by cache-busting.
        src += (src.indexOf('?') === -1 ? '?' : '&') + 'v=20260227';
    }
